...
const AnnualTokenTax = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/files/vehicles-tokentaxes-annualtokentax/${encodedZone}/${encodedBranch}`);
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/files/vehicles-tokentaxes-annualtokentax/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `http://localhost:5000/api/files/vehicles-tokentaxes-annualtokentax/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
