...
const AdminPolicies = ({ open, user }) => {
  ...
  const fetchZones = async () => {
    try {
      ...
      console.log("Zones Response:", response);  // Log the response
      const zonesData = await response.json();
      console.log("Zones Data:", zonesData);  // Log the response data
      setZones(zonesData);
    } catch (error) {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      // Ensure this endpoint is correct in the backend
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/adminpolicies/${encodedZone}/${encodedBranch}`);
  
      if (response.ok) {
        const filesData = await response.json();
        console.log("Fetched Files Data:", filesData);  // Log files to see if they are fetched correctly
        setFiles(filesData);
      } else {
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/adminpolicies/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/adminpolicies/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
  ...
  return (
    ...
    <Typography
      ...
      sx={{
        ...
        mb: 4,
        textAlign: 'center',
        fontSize: { xs: '24px', sm: '30px' }, // Adjust font size for mobile
        fontFamily: 'TanseekModernW20',
        borderBottom: '2px solid #ccc',
        paddingBottom: '10px',
      }}
    >
      ADMIN POLICIES AND SOP's
    </Typography>
    <Typography
      variant="subtitle1"
      sx={{
            ...
            <Grid item xs={12} sm={3}>
              <FormControl fullWidth>
                <SearchBar
                  ...
                  style={{ width: '100%' }}  // Make search bar responsive
                />
              </FormControl>
            </Grid>
            ...
            <Grid item xs={12} sm={9}>
              <FormControl fullWidth>
                <SearchBar
                  ...
                  setSearchQuery={setSearchQuery}
                  placeholder="Search files..."
                  style={{ width: '100%' }} // Make search bar full width on mobile
                />
              </FormControl>
            </Grid>
