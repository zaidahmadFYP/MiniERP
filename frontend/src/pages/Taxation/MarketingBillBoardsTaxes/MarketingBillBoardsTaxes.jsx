...
const MarketingBillBoardsTaxes = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/taxation-marketingbillboardstaxes/${encodedZone}/${encodedBranch}`);
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/taxation-marketingbillboardstaxes/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/taxation-marketingbillboardstaxes/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
  ...
  >
    TAXATION/MARKETING AND BILLBOARD TAXES
  </Typography>
  <Typography
    variant="subtitle1"
    sx={{
...
export default MarketingBillBoardsTaxes;