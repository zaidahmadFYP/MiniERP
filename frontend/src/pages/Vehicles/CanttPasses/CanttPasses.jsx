...
const CanttPasses = ({ open, user }) => {
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/vehicles-routepermits-canttpasses/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
