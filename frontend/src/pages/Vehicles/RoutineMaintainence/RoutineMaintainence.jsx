...
const RoutineMaintainence = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/vehicles-maintenance-routinemaintenance-oilchangetuning/${encodedZone}/${encodedBranch}`);
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/vehicles-maintenance-routinemaintenance-oilchangetuning/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/vehicles-maintenance-routinemaintenance-oilchangetuning/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
  ...
  return (
    <MainContentWrapper open={open}>
      ...
      >
        VEHICLES/MAINTENANCE/ROUTINE MAINTENANCE/OIL CHANGE AND TUNING
      </Typography>
      <Typography
        variant="subtitle1"
        sx={{
...
export default RoutineMaintainence;