...
const DineIn = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/approvals-outerspaces-dinein/${encodedZone}/${encodedBranch}`);
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/approvals-outerspaces-dinein/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/approvals-outerspaces-dinein/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
  ...
  <Typography
    ...
    sx={{
      ...
      paddingBottom: '10px',
      transition: 'all 0.3s ease-in-out', // Smooth transition for color
    }}
  >
    LICENSES/OUTER SPACES/DINE IN
  </Typography>
  <Typography
    variant="subtitle1"
    sx={{
  ...
  >
    ...
    <Box
      sx={{
        width: '100%',
        maxHeight: '500px',
        overflowY: files.length > 5 ? 'auto' : 'unset',
