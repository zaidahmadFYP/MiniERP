...
const RecurringMedicalTests = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/licenses-staffmedicals-rmt/${encodedZone}/${encodedBranch}`);
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/licenses-staffmedicals-rmt/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/licenses-staffmedicals-rmt/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
  ...
  return (
    <MainContentWrapper open={open}>
      ...
      >
        LICENSES/STAFF MEDICALS/RECURRING MEDICAL TESTS
      </Typography>
      <Typography
        variant="subtitle1"
        sx={{
...
export default RecurringMedicalTests;