...
const LabourLicenses = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/labourlicenses/${encodedZone}/${encodedBranch}`);
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/labourlicenses/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/labourlicenses/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
