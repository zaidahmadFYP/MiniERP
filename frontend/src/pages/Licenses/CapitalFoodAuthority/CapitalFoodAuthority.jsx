...
const CapitalFoodAuthority = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/licenses-tradelicenses-cda/${encodedZone}/${encodedBranch}`);
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/licenses-tradelicenses-cda/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/licenses-tradelicenses-cda/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
  ...
  return (
    ...
    <Typography
      ...
      sx={{
        ...
        paddingBottom: '10px',
        transition: 'color 0.3s, border-bottom 0.3s',  // Transition for theme switching
      }}
    >
      LICENSES/TRADE LICENSES/CAPITAL DEVELOPMENT AUTHORITY
    </Typography>
    ...
    <Typography
      ...
      sx={{
        ...
        mb: 2,
        transition: 'color 0.3s', // Transition for theme switching
      }}
    >
    ...
    <Box
      sx={{
        ...
        padding: '20px',
        borderRadius: '8px',
        transition: 'background-color 0.3s', // Transition for background color on theme switch
      }}
    >
      ...
      <Box
        sx={{
          ...
          padding: '20px',
          borderRadius: '8px',
          transition: 'background-color 0.3s, color 0.3s', // Transition for theme switching
        }}
      >
        {loading ? (
...
export default CapitalFoodAuthority;