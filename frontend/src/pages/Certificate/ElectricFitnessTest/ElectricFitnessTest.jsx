...
const ElectricFitnessTest = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/certificates-electricfitnesstest/${encodedZone}/${encodedBranch}`);
  ...
  const handleFileSelect = async (file) => {
    if (file) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/certificates-electricfitnesstest/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`, {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDeleteConfirm = async () => {
    ...
    const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/certificates-electricfitnesstest/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
  ...
  return (
    ...
    >
      CERTIFICATES/ELECTRIC FITNESS TESTS
    </Typography>
    <Typography
      variant="subtitle1"
      sx={{
            ...
            <Grid item xs={12} sm={10}>
              <FormControl fullWidth>
                <SearchBar
                  ...
                  setSearchQuery={setSearchQuery}
                  placeholder="Search files..."
                  style={{ width: '100%' }} // Take full width on mobile
                />
              </FormControl>
            </Grid>
...
export default ElectricFitnessTest;